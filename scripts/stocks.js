...
JobOrder.prototype.saveStock = function(e) {
	...
	if (document.getElementById('stock-no').value && this.checkSignedInWithMessage()) {
		...
		this.stocksRef.push({
			...
			stockNo:document.getElementById('stock-no').value,
			stockPaperType:document.getElementById('stock-paper-type').value,
			stockPaperCount:document.getElementById('stock-paper-count').value,
			stockPaperWeight:document.getElementById('stock-paper-weight').value,
			stockPaperSize:document.getElementById('stock-paper-size').value,
			stockPaperUnitPrice:document.getElementById('stock-paper-unit-price').value,
			stocksUsedCount: 0
...
JobOrder.prototype.displayStock = function(key, stockNo, stockDate, stockDateDesc, stockJobOrderNo, stockPaperType, stockPaperCount, stockPaperUsed, stockPaperWeight, stockPaperSize, stockPaperUnitPrice) {
	...
	div.querySelector('.consumeButton').addEventListener('click', function() {
		...
		var consumeFill = function(snap){
			...
			document.getElementById('stocks-used').innerHTML = JobOrder.STOCK_USED_TEMPLATE
...
//	setTimeout(function() {div.classList.add('visible')}, 1)
	document.getElementById('stocks').scrollTop = document.getElementById('stocks').scrollHeight
	document.getElementById('stock-no').focus()
}
